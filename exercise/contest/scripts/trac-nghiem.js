...
export function randomTracNghiem(id) {
    switch(id) {
        ...
        case 20: { // Tìm phương trình đường thẳng
            ...
            case 21: { 
    let question = "Điểm giao nhau của hai đường thẳng y = ax + b và y = cx + d là gì?";
    
    let a = randomInRange(-10, 10);
    let b = randomInRange(-10, 10);
    let c = randomInRange(-10, 10);
    while (c === a) {  // Đảm bảo hai hệ số góc không bằng nhau
        c = randomInRange(-10, 10);
    }
    let d = randomInRange(-10, 10);
    ...
    let answerChoices = [
        `(${x.toFixed(2)}, ${y.toFixed(2)})`,  // Đáp án đúng
        `(${(x + 1).toFixed(2)}, ${(y + 1).toFixed(2)})`,
        `(${(x - 1).toFixed(2)}, ${(y - 1).toFixed(2)})`,
        `(${(x + 2).toFixed(2)}, ${(y - 2).toFixed(2)})`
    ];

    let explain = `Giao điểm của hai đường thẳng được tìm bằng cách giải hệ phương trình: y = ax + b và y = cx + d`;

    return { question, answerChoices, explain };
    
    }
    case 22: { // Giá trị x = 3 là nghiệm của bất phương trình nào
        const x = 3;
    
        const question = `
            Giá trị x = ${x} là một nghiệm của phương trình nào sau đây?
        `;
        ...
        const explain = `
            Thử từng phương trình với x = ${x}:<br><br>
    
            A. 2×${x}² = 18; 5 + ${x} = 8 → 18 ≤ 8 ❌<br>
            B. -4×${x} + 7 = -5; -1 - ${x} = -4 → -5 > -4 ❌<br>
            <b>C. 3×${x} - 8 = 1; -7×${x} = -21 → 1 ≥ -21 ✅</b><br>
            D. 5 - 3×${x} = -4; -4×${x} + 2 = -10 → -4 < -10 ❌<br><br>
    ...
    case 23: {
        const question = `
            Không giải hệ phương trình, dự đoán số nghiệm của hệ:<br>
            \\[
            \\begin{cases}
            -x + 5y = -1 \\\\
            5x + y = 2
        ...
        const explain = `
            ...
            -x + 5y = -1 &nbsp;&nbsp;&nbsp;(1)<br>
            5x + y = 2 &nbsp;&nbsp;&nbsp;(2)<br><br>
            - Từ (1): hệ số x = -1, y = 5<br>
            - Từ (2): hệ số x = 5, y = 1<br>
            → Hệ số x và y **không tỷ lệ** → hai phương trình không đồng biến<br>
            Hệ **có nghiệm duy nhất**<br><br>
            ✅ Vậy đáp án đúng là: <b>C</b>
        `;
